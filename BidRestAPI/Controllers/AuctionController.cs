...
namespace BidRestAPI.Controllers
{
    ...
    public class AuctionController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteAuction(Guid id)
        {
            try
            {
                var userId = User.FindFirstValue(ClaimTypes.NameIdentifier); // Extract the user ID from claims
                if (string.IsNullOrEmpty(userId))
                {
                    _logger.LogWarning("Delete auction failed: User not found or invalid token.");
                ...
                _logger.LogInformation($"Auction with ID {id} deleted successfully by user {userId}.");
                return Ok(new { message = "Auction deleted successfully." });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error occurred while deleting auction with ID {id}.");
