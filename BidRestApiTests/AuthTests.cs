...
namespace BidRestAPI.Tests
{
    ...
    public class AuthTests
    {
        ...
        {
            // Arrange
            var dto = new UserRegisterDto(); // Invalid DTO
        ...
        [Test]
        public async Task Register_WhenExceptionThrown_ShouldReturnInternalServerError()
        {
            // Arrange
            var dto = new UserRegisterDto { Email = "test@example.com", Password = "Password123" };
            _authServiceMock.Setup(auth => auth.RegisterAsync(dto)).ThrowsAsync(new Exception("Error during registration"));
